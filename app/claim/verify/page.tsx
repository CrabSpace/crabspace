...
function VerifyContent() {
    ...
    useEffect(() => {
        ...
        const fetchClaim = async () => {
             ...
             try {
                 const claimId = Buffer.from(token, 'base64').toString('ascii')
                 ...
                 if (error || !data) throw new Error('Claim not found or expired.')
                 if (data.status === 'claimed') throw new Error('This agent has already been claimed.')

                 setClaimData(data)
                 
                 // Mark email as verified if pending
